...
/**
 * @author dingdj Date:2014-7-10上午9:45:54
 * 
 */
public class ViewPager extends ViewGroup implements ISlidingConflict {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (isFirstLayout) {
			if (init != -1 && init >= 0 && init < getChildCount())
	...
	public void snapToScreen(int whichScreen) {
		if (whichScreen >= getChildCount()
				&& onTouchedLastPageListenner != null) {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (isc != null) {
				isc.setChildSlidingViewState(true, false, getChildCount(),
						mCurScreen);
			}
			...
			if (isHorizontalSlidingInChild && Math.abs(xDiff) > mTouchSlop) {
				...
				if (xDiff < 0 && childViewCurrentScreen == 0) {
					return true;
						...
						&& childViewCurrentScreen == childViewTotalScreen - 1) {
					return true;
				} else {
					return false;
				}
	 ...
	 */
	public void setOnPageSelectedListenner(
			OnPageSelectedListenner onPageSelectedListenner) {
	 ...
	 */
	public void setOnTouchedLastPageListenner(
			OnTouchedLastPageListenner onTouchedLastPageListenner) {
		this.onTouchedLastPageListenner = onTouchedLastPageListenner;
	}

	@Override
	public void setChildSlidingViewState(boolean isHorizontalSlidingInChild,
			boolean isChildEndlessScrolling, int childViewTotalScreen,
			int childViewCurrentScreen) {
		if (childViewTotalScreen <= 0 || childViewCurrentScreen < 0
				|| childViewCurrentScreen >= childViewTotalScreen) {
			return;
		}
